...
const members = [
  { initials: 'JT', name: 'Jamie Tan', uni: 'NUS · CS Year 2', role: 'Frontend Lead', gradient: 'linear-gradient(135deg,#ff6b6b,#ff8a65)', skills: ['React', 'TypeScript', 'Figma'], progress: 87 },
  { initials: 'WM', name: 'Wei Ming Chen', uni: 'NTU · CE Year 3', role: 'ML Engineer', gradient: 'linear-gradient(135deg,#42a5f5,#1e88e5)', skills: ['Python', 'TensorFlow', 'AWS'], progress: 91 },
  { initials: 'EH', name: 'Emily Huang', uni: 'SMU · IS Year 3', role: 'Full-Stack', gradient: 'linear-gradient(135deg,#ff8a65,#ff6b6b)', skills: ['Node.js', 'React', 'PostgreSQL'], progress: 78 },
  { initials: 'PS', name: 'Priya Sharma', uni: 'SMU · IS Year 2', role: 'Backend Dev', gradient: 'linear-gradient(135deg,#b39ddb,#7e57c2)', skills: ['Node.js', 'Docker', 'PostgreSQL'], progress: 82 },
];

const chatMessages = [
  { initials: 'JT', gradient: 'linear-gradient(135deg,var(--accent),var(--peach))', text: 'Just pushed the swipe animation update! Check the PR when you can 🎨', self: false },
  ...
  { initials: 'EH', gradient: 'linear-gradient(135deg,var(--peach),var(--accent))', text: "Amazing work both of you! I'll review the PRs tonight", self: true },
...
const sprint = [
  { emoji: '✅', title: 'API Matching Algorithm', sub: 'Assigned to Wei Ming · Completed' },
  { emoji: '🔄', title: 'Swipe UI Component', sub: 'Assigned to Jamie · In Progress' },
  { emoji: '📝', title: 'Database Migration', sub: 'Assigned to Priya · In Progress' },
  { emoji: '⏳', title: 'Pitch Deck', sub: 'Assigned to You · Pending' },
];
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <div style={{ position: 'relative', zIndex: 2, padding: '100px 40px 60px', maxWidth: 1300, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 32 }}>
          <div style={{ padding: 24, borderRadius: 'var(--radius)', background: 'var(--bg-card)', boxShadow: 'var(--shadow-card)', border: '1px solid rgba(0,0,0,0.04)' }}>
            ...
            <div style={{ display: 'flex', gap: 20 }}>
              {[{ val: '4', lbl: 'Members' }, { val: '3', lbl: 'Hackathons' }, { val: '4.8', lbl: 'Avg Rating' }, { val: '2', lbl: 'Wins' }].map(s => (
                <div key={s.lbl} style={{ textAlign: 'center' }}>
                  <span style={{ fontSize: '1.3rem', fontWeight: 800, display: 'block' }}>{s.val}</span>
          ...
          <div style={{ padding: 24, borderRadius: 'var(--radius)', background: 'var(--bg-card)', boxShadow: 'var(--shadow-card)', border: '1px solid rgba(0,0,0,0.04)' }}>
            ...
            {sprint.map(s => (
              <div key={s.title} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: 14, borderRadius: 16, marginBottom: 8, background: 'var(--bg)' }}>
                <div style={{ width: 40, height: 40, borderRadius: 12, background: 'var(--bg-warm)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.1rem' }}>{s.emoji}</div>
                <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16, marginBottom: 32 }}>
          {members.map(m => (
            <div key={m.name} style={{ padding: 22, borderRadius: 20, background: 'var(--bg-card)', boxShadow: 'var(--shadow-card)', border: '1px solid rgba(0,0,0,0.04)', transition: 'transform 0.3s, box-shadow 0.3s' }}
            ...
            >
              ...
              <span style={{ display: 'inline-block', padding: '4px 12px', borderRadius: 9, fontSize: '0.65rem', fontWeight: 700, marginBottom: 12, background: 'var(--accent-light)', color: 'var(--accent)' }}>{m.role}</span>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div style={{ flex: 1, height: 6, borderRadius: 3, background: 'var(--bg)', overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${m.progress}%`, borderRadius: 3, background: 'var(--accent)', transition: 'width 1s' }} />
                </div>
                <span style={{ fontSize: '0.65rem', fontWeight: 700, fontFamily: "'Fira Code', monospace", color: 'var(--text-soft)' }}>{m.progress}%</span>
              </div>
            </div>
          ))}
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {chatMessages.map((m, i) => (
              <div key={i} style={{ display: 'flex', gap: 10, flexDirection: m.self ? 'row-reverse' : 'row' }}>
                ...
                <div style={{ padding: '10px 16px', borderRadius: 16, fontSize: '0.8rem', lineHeight: 1.6, maxWidth: '75%', background: m.self ? 'var(--accent)' : 'var(--bg)', color: m.self ? 'white' : 'var(--text-body)', borderTopRightRadius: m.self ? 4 : 16, borderTopLeftRadius: m.self ? 16 : 4 }}>{m.text}</div>
              </div>
            ))}
            <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
              ...
              <button style={{ width: 44, height: 44, borderRadius: 14, border: 'none', background: 'var(--accent)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
