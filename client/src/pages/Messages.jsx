          ...
          <div style={{ borderRadius: 'var(--radius)', background: 'var(--bg-card)', boxShadow: 'var(--shadow-card)', border: '1px solid rgba(0,0,0,0.04)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto' }}>
              {conversations.map((c, i) => (
                <div key={c.name} onClick={() => setActive(i)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '16px 18px', cursor: 'pointer', transition: 'background 0.2s', borderLeft: `3px solid ${active === i ? 'var(--accent)' : 'transparent'}`, background: active === i ? 'rgba(224,93,80,0.04)' : 'transparent' }}>
                  ...
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    ...
                    {c.unread > 0 && <div style={{ width: 18, height: 18, borderRadius: '50%', background: 'var(--accent)', color: 'white', fontSize: '0.55rem', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', marginLeft: 'auto' }}>{c.unread}</div>}
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ borderRadius: 'var(--radius)', background: 'var(--bg-card)', boxShadow: 'var(--shadow-card)', border: '1px solid rgba(0,0,0,0.04)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '18px 24px', borderBottom: '1px solid rgba(0,0,0,0.05)' }}>
              ...
              <div>
                ...
                <p style={{ fontSize: '0.7rem', color: conversations[active].online ? '#5a9a5e' : 'var(--text-soft)', fontWeight: 500 }}>{conversations[active].online ? 'Online now' : 'Offline'}</p>
              </div>
            </div>
